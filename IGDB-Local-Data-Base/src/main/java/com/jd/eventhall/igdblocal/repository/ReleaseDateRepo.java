...
public class ReleaseDateRepo {
    ...
    public boolean insertReleaseDate(ReleaseDate releaseDate) {
        Document doc = new Document("_id", releaseDate.getId())
        .append("date", releaseDate.getDate().getSeconds()*1000l)
        .append("game", releaseDate.getGame().getId())
        .append("human", releaseDate.getHuman())
        .append("platform", releaseDate.getPlatform().getId())
        .append("region", releaseDate.getRegionValue());
    ...
    public boolean updateReleaseDate(ReleaseDate releaseDate) {
        // Document doc = new Document().toJson();
        Query query = new Query(Criteria.where("_id").is(releaseDate.getId()));
        Update update = new Update()
        .set("date", releaseDate.getDate().getSeconds()*1000l)
        .set("game", releaseDate.getGame().getId())
        .set("human", releaseDate.getHuman())
        .set("platform", releaseDate.getPlatform().getId())
        .set("region", releaseDate.getRegionValue());
